...
export default function Press() {
  ...
  const load = async (p = 1, reset = false) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || 'Failed to load news articles';
      setError(errorMsg);
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', backgroundColor: '#F5F0EB', color: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', backgroundColor: '#F5F0EB', color: '#0A0A0A', padding: '40px 20px' }}>
  ...
  return (
    <div className="press-page" style={{ backgroundColor: '#F5F0EB', minHeight: '100vh', color: '#0A0A0A' }}>
