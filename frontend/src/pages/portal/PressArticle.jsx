...
const GallerySlideshow = ({ gallery, coverImage }) => {
  // Combine cover + gallery photos
  const allImages = [
    ...(coverImage ? [coverImage] : []),
    ...(gallery || [])
  ].filter(img => img && img.url); // Remove any undefined URLs
  ...
  return (
    <div className="article-slideshow" style={{ position: 'relative', width: '100%', marginBottom: '40px' }}>
          ...
          {allImages.map((_, idx) => (
            <button
              ...
              style={{
                ...
                borderRadius: '50%',
                border: 'none',
                background: idx === currentIndex ? '#C9A96E' : '#D0D0D0',
                cursor: 'pointer',
      ...
      <article className="press-article" style={{ maxWidth: '900px', margin: '0 auto', padding: '0 20px', paddingBottom: '80px' }}>
        ...
        {article.gallery && article.gallery.length > 0 && (
          <div className="article-gallery" style={{ marginTop: '60px', marginBottom: '60px' }}>
            ...
            <GallerySlideshow gallery={article.gallery} coverImage={null} />
          </div>
        )}
